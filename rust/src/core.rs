...
pub fn institutional_readiness(
    ...
    let readiness =
        0.28 * (1.0 - extraction) + 0.28 * openness + 0.26 * adaptability + 0.18 * capital_scale;
...
pub fn calculate_all_network_bonuses(
    ...
    for i in 0..n {
        ...
        if w > 1e-9 {
            ...
            cache.network_bonuses[i] = cache.openness[i]
                * local_market
                * (1.0 + config.network_density_gain * local_density);
        } else {
...
pub fn compute_all_rewards(
    ...
    for i in 0..n {
        ...
        let active_steps = (state.population - 1).max(0);
        ...
        let resource_payoff = resource_rents
            * (config.resource_base_payoff + config.resource_capture_gain * extractions);
        ...
        let mid_city_multiplier =
            (-(((pop + 1.0).ln() - target_log).powi(2)) / (2.0 * spread * spread)).exp();
        let secondary_city_dividend =
            config.secondary_city_bonus * network_bonus * mid_city_multiplier;
        ...
        let reform_cost = if reform_timers > 0 {
            config.reform_cost
        } else {
            0.0
        };
    ...
    for state in states {
        cache.weights.push(
            state
                .resource_rent
                .max(1e-6)
                .powf(1.0 + config.shock_target_resource_bias),
        );
    }
...
pub fn evolve_sites<R: Rng + ?Sized>(
    ...
    let mean_reward = if n > 0 {
        cache.rewards.mean().unwrap_or(0.0)
    } else {
        0.0
    };
    ...
    for index in 0..n {
        ...
        let transition_factor =
            if config.shock_transition_duration > 0 && state.post_shock_timer > 0 {
        ...
        let legacy_support = legacy_factor
        ...
        let effective_adaptability = (state.institution.adaptability
        ...
        if state.institution.reform_timer == 0 {
            ...
            if rng.gen::<f64>() < reform_probability {
                ...
                state.institution.extraction =
                    (state.institution.extraction - config.reform_step * reform_scale).max(0.0);
                state.institution.openness = (state.institution.openness
                    + config.reform_openness_gain
                    + config.shock_openness_bonus * shock_factor)
                    .min(1.0);
                state.institution.reform_timer = config.reform_duration;
                state.reforms_triggered += 1;
                state.productive_capital = (state.productive_capital
                    + config.shock_capital_rebuild * shock_factor)
                    .min(1.5);

                if shock_factor > 0.0 || transition_factor > 0.0 {
                    state.shock_reform_stock = state
                        .shock_reform_stock
                        .max(
                            config.reform_openness_gain
                                + 0.75 * readiness * shock_factor.max(transition_factor),
                        )
                        .min(1.0);
                }
            } else {
                let curse_modifier = (1.0
                    ...
                    .max(0.1);
                state.institution.extraction = (state.institution.extraction
        ...
        if transition_factor > 0.0 {
            state.institution.extraction = (state.institution.extraction
                - config.shock_transition_extraction_decay * transition_support)
                .max(0.0);
            extraction_cap =
                1.0 - (1.0 - config.shock_transition_extraction_cap) * transition_support.min(1.0);
            state.institution.extraction = state.institution.extraction.min(extraction_cap);
            state.institution.openness = (state.institution.openness
                + (config.shock_transition_openness_gain
                    + 0.45 * config.shock_lock_in_bonus * readiness)
                    * transition_support)
                .min(1.0);
            state.productive_capital = (state.productive_capital
                + (config.shock_transition_capital_gain
                    + 0.6 * config.shock_lock_in_bonus * readiness)
                    * transition_support)
                .min(1.5);
        ...
        if legacy_factor > 0.0 {
            state.institution.extraction = (state.institution.extraction
                - config.shock_legacy_extraction_decay * legacy_support)
                .max(0.0);
            state.institution.openness = (state.institution.openness
                + (config.shock_legacy_openness_gain
                    + 0.25 * config.shock_lock_in_bonus * readiness)
                    * legacy_support)
                .min(1.0);
        }

        let capital_investment = 0.25
            * config.inclusive_investment_gain
            * state.resource_rent
            * (1.0 - state.institution.extraction)
            * (0.4 + state.institution.openness + 0.25 * network);
        let capital_erosion = config.extractive_capital_erosion
            * state.institution.extraction
            * (0.35 + state.resource_rent);

        let passive_scale = if Some(index) == active_site {
            1.0
        } else {
            config.passive_investment_scale
        };

        state.productive_capital =
            (state.productive_capital + passive_scale * capital_investment + 0.015 * network
                - capital_erosion)
                .clamp(0.0, 1.5);

        if Some(index) == active_site {
            let active_steps = (state.population - 1).max(0) as f64;
            let mut activity_load = ((active_steps - config.active_decay_onset as f64)
                / (config.active_decay_onset.max(1) as f64))
                .max(0.0);
            ...
            let legacy_brake =
                (1.0 - config.shock_legacy_extraction_brake * legacy_support).max(0.0);
            ...
            state.institution.extraction =
                (state.institution.extraction + extraction_pressure).min(1.0);
            if transition_factor > 0.0 {
                state.institution.extraction = state.institution.extraction.min(extraction_cap);
            }
        ...
        if state.first_shock_step.is_some() && transition_factor <= 0.0 {
            let lock_in =
                config.shock_lock_in_bonus * readiness * state.productive_capital.min(1.0);
            let snapback = config.shock_snapback_pressure
                * (1.0 - readiness)
                * state.resource_rent
                * (1.0 - state.institution.openness);
            state.institution.extraction =
                (state.institution.extraction + snapback - lock_in).clamp(0.0, 1.0);
            state.institution.openness =
                (state.institution.openness + 0.35 * lock_in - 0.15 * snapback).clamp(0.0, 1.0);
        }

        state.institution.openness = (state.institution.openness
            + config.network_openness_influence * (network - state.institution.openness))
            .clamp(0.0, 1.0);
        ...
        let overstretch_ratio =
            (state.population as f64 - config.metropolitan_overstretch_threshold).max(0.0)
                / config.metropolitan_overstretch_threshold.max(1.0);
        let structural_signal = geography + 0.45 * network + 0.35 * state.productive_capital;

        let growth_signal = reward_signal
            + config.growth_structural_weight * (structural_signal - 0.75)
            + 0.45 * (mid_city_signal - 0.35);
        let momentum_delta = config.endogenous_growth_rate * growth_signal
            - config.endogenous_decline_rate * overstretch_ratio;

        state.population_momentum =
            config.endogenous_growth_decay * state.population_momentum + momentum_delta;

        if legacy_factor > 0.0 && config.shock_legacy_fade > 0.0 {
            state.shock_reform_stock =
                (state.shock_reform_stock - config.shock_legacy_fade).max(0.0);
        }

        if state.first_shock_step.is_some()
            && state.institution.extraction < state.initial_extraction
        {
    ...
    if config.shock_probability > 0.0 && rng.gen::<f64>() < config.shock_probability {
        ...
        states[choice].resource_rent =
            (states[choice].resource_rent * (1.0 - config.depletion_rate)).max(0.0);
        states[choice].productive_capital =
            (states[choice].productive_capital - 0.3 * config.depletion_rate).max(0.0);
        ...
        states[choice].post_shock_timer = states[choice]
            .post_shock_timer
            .max(config.shock_transition_duration);

        if config.shock_reform_bonus > 0.0 {
            let readiness = states[choice].shock_readiness;
            let impulse = readiness;
            if impulse > 0.0 {
                states[choice].institution.adaptability = (states[choice].institution.adaptability
                    + 0.35 * config.shock_transition_adaptability_bonus * impulse)
                    .min(1.0);
                states[choice].shock_reform_stock = (0.08 + 0.82 * impulse)
                    .max(states[choice].shock_reform_stock)
                    .min(1.0);
                states[choice].institution.extraction = (states[choice].institution.extraction
                    - 0.45
                        * config.reform_step
                        * (1.0 + config.shock_reform_step_bonus)
                        * (0.25 + impulse)
                    - 0.5 * config.shock_transition_extraction_decay * impulse)
                    .max(0.0);
                states[choice].institution.openness = (states[choice].institution.openness
                    + config.shock_openness_bonus * impulse)
                    .min(1.0);
                states[choice].productive_capital = (states[choice].productive_capital
                    + config.shock_capital_rebuild * impulse)
                    .min(1.5);
                states[choice].reforms_triggered += 1;
                states[choice].institution.reform_timer = states[choice]
                    .institution
                    .reform_timer
                    .max((config.reform_duration / 2).max(1));
            }
        }
        shock_site = Some(choice);
    }

    EvolveReport { shock_site }
}
